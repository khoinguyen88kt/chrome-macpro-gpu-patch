...
int main(int argc, char *argv[]) {
    ...
    snprintf(fw_path, sizeof(fw_path), 
        "%s/../Frameworks/Helium Framework.framework/Helium Framework", dir);
    ...
    const char **new_argv = (const char **)malloc(sizeof(char *) * (new_argc + 1));
    new_argv[0] = argv[0];
    ...
    for (int f = 0; f < add_count; f++) {
        new_argv[idx++] = to_add[f];
    }
    new_argv[new_argc] = NULL;

    return chrome_main(new_argc, new_argv);
}
