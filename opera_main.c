...
int main(int argc, char *argv[]) {
    ...
    snprintf(versions_dir, sizeof(versions_dir), 
        "%s/../Frameworks/Opera Framework.framework/Versions", dir);

    DIR *d = opendir(versions_dir);
    if (!d) {
        fprintf(stderr, "Failed to open versions dir: %s\n", versions_dir);
        return 1;
    }
    struct dirent *ent;
    char best_ver[256] = "";
    while ((ent = readdir(d)) != NULL) {
        if (ent->d_name[0] != '.' && strcmp(ent->d_name, "Current") != 0) {
            if (strcmp(ent->d_name, best_ver) > 0) {
    ...
    snprintf(fw_path, sizeof(fw_path), 
        "%s/%s/Opera Framework", versions_dir, best_ver);
